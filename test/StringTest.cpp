...
TEST_CASE("StringDefaultConstructor", "") {
    ...
    string stringResult = validString.toString();
    string stringExpect = (string) "Huynh";

    // Check valid string default constructor - Should equals
    CHECK(result == expect);
    CHECK(strcmp(stringExpect, stringResult) == 0);
}
...
TEST_CASE("StringReplace", "") {
    ...
    validString.replace(6, 1, "212341233411111111111");
    string stringResult = validString.toString();
    string stringExpect = (string) "122567212341233411111111111";
    CHECK(strcmp(stringExpect, stringResult) == 0);

    validString = "123456";
    validString.replace(0, validString.getSize(), "654321");
    stringExpect = (string) "654321";
    stringResult = validString.toString();
    CHECK(strcmp(stringExpect, stringResult) == 0);

    validString = "00000";
    validString.replace(1, 2, "1");
    stringExpect = (string) "0100";
    stringResult = validString.toString();
    CHECK(strcmp(stringExpect, stringResult) == 0);
}